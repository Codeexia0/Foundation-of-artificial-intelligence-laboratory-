...
class Space:
    ...
    def generate_and_plot_heatmap(self):
        """Generate a heatmap based on Manhattan distance from houses and plot it."""
        ...
        for i in range(self.height):
            for j in range(self.width):
                for house in self.houses:
                    # If there are houses at positions (1,1) and (3,4) and the value at heatmap[2,2] would be:
                    # abs(1-2) + abs(1-2) + abs(3-2) + abs(4-2) = 1 + 1 + 1 + 2 = 5
                    heatmap[i, j] += abs(house[0] - i) + abs(house[1] - j)
                    # print("heat", i, j, heatmap, "Sum: ", sum(heatmap), "\end") # Debugging
